...
public class NoteDatabase {
    ...
    public void insert(NotebookData data) {
        ...
        sqlite.execSQL(sql, new String[] { data.id + "",
                data.iid + "", data.timestamp + "", data.updateTime,
                data.content, data.getColor() + "" });
        sqlite.close();
    }
    ...
    public void update(NotebookData data) {
        ...
        sqlite.execSQL(sql,
                new String[] { data.iid + "", data.timestamp + "",
                        data.updateTime, data.content,
                        data.getColor() + "", data.id + "" });
        sqlite.close();
    }
    ...
    public List<NotebookData> query(String where) {
        ...
        for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
            ...
            notebookData.id = cursor.getInt(0);
            notebookData.iid = cursor.getInt(1);
            notebookData.setUnixTime(cursor.getString(2));
            notebookData.updateTime = cursor.getString(3);
            notebookData.content = cursor.getString(4);
    ...
    public void save(NotebookData data) {
        List<NotebookData> datas = query(" where _id=" + data.id);
